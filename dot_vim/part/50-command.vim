...
command! -bang -complete=file -nargs=? Sjis edit<bang> ++enc=sjis <args>
...
command! -bang -complete=file -nargs=? WSjis write<bang> ++enc=sjis <args>
...
function! s:rename(filename) abort
  execute 'save' a:filename
  call delete(expand('#'))
...
function! s:mfc_calc(expr)
  let l:result = trim(substitute(system('mfc ' . shellescape(a:expr) . ' println'), '^$', '', 'g'))
  call anekos#util#insert_word(l:result)
endfunction
...
function! s:show_something()
  let l:message = [
  \  'Filename  = ' . expand('%'),
  \  'LSP       = ' . fnamemodify(lsp_settings#root_path([]), ':~'),
  \  'CWD       = ' . fnamemodify(getcwd(), ':~'),
  \  'Filetype  = ' . &filetype,
  \  'Line      = ' . line('.') . '/' . line('$'),
  \  'Encoding  = ' . &fileencoding,
  \  'Modified  = ' . strftime('%Y-%m-%d %H:%M:%S', getftime(expand('%'))),
  \ ]
  call popup_create(l:message, {'padding': [], 'moved': 'any'})
endfunction

command! ShowSomething call s:show_something()
...
function! s:CopyBufferToClipboard()
  ...
  for l:ln in range(1, line('$'))
    call add(l:lines, getline(l:ln))
  endfor

  let @* = join(l:lines, "\n")
endfunction

function! s:AutoCopyBuffer()
  augroup AutoCopyBuffer
    autocmd!
    autocmd BufWritePost <buffer> CopyBufferToClipboard
  augroup end
endfunction

command! CopyBufferToClipboard call s:CopyBufferToClipboard()
command! AutoCopyBuffer call s:AutoCopyBuffer()
